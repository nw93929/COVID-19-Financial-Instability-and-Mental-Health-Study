...
With a categorical predictor, there is an additional assumption to check: that the variance of the response variable is equal across classes of the categorical predictor
```{r}
library(lawstat)
lawstat::levene.test(data.no.wt$SAT_total,data$RACEREC)
lawstat::levene.test(data.no.wt$SAT_total,data$SEX)
lawstat::levene.test(data.no.wt$SAT_total,data$EDUC)
lawstat::levene.test(data.no.wt$SAT_total,data$HOUSING_2)
lawstat::levene.test(data.no.wt$SAT_total,data$MARSTAT)
lawstat::levene.test(data.no.wt$SAT_total,data$EMP)
```
The null hypothesis is that the variance of the response variable is equal across all classes. 
...
```
Forward and backward selection select the same sets of variables. We will continue with the model: SAT_total ~ SEX + EDUC + EMP + MARSTAT + HOUSING_2 + 
    SLEEPHRS + PHYS_HEALTH + HHINC + MENT_HEALTH + AGE
...
```{r}
fin_vars <- c("COV_EXTUI", "COV_STIMULUS", "MEDBILLS", "MONEYFOOD", "CREDITDEBIT") 
demoFin_result <- lm(SAT_total~SEX + EDUC + EMP + MARSTAT + HOUSING_2 + 
    SLEEPHRS + PHYS_HEALTH + HHINC + MENT_HEALTH + AGE + COV_EXTUI+COV_STIMULUS+COV_MEDBILLS+ COV_MONEYFOOD+COV_CREDITDEBT, data = data.no.wt, weights = data$FINAL_WGT)
...
```{r}
reducedFindemo <- lm(SAT_total~SEX + EDUC + EMP + MARSTAT + HOUSING_2 + 
    SLEEPHRS + PHYS_HEALTH + HHINC + MENT_HEALTH + AGE + COV_MONEYFOOD+COV_CREDITDEBT, data = data.no.wt, weights = data$FINAL_WGT)
