...
```
Forward and backward selection select the same sets of variables. We will continue with the model: SAT_total ~ MENT_HEALTH + PHYS_HEALTH + MARSTAT + HOUSING_2 +
...
SEX + AGE + SLEEPHRS + HHINC + EDUC + EMP + MILITARY + COV_EXTUI+COV_STIMULUS+COV_MEDBILLS+ COV_MONEYFOOD+COV_CREDITDEBT, data = data.no.wt, weights = data$FINAL_WGT)
...
SEX + AGE + SLEEPHRS + HHINC + EDUC + EMP + MILITARY + COV_MONEYFOOD+COV_CREDITDEBT+COV_EXTUI, data = data.no.wt, weights = data$FINAL_WGT)
